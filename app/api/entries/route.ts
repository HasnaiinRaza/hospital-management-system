...
export async function GET(req: NextRequest) {
  ...
  let query = supabaseAdmin
    ...
    .select(`
      *,
      doctor:users!patient_entries_doctor_id_fkey(id, name, email, department_id),
...
export async function POST(req: NextRequest) {
  const {
    ...
    male_count, female_count, pediatric_count,
    status, remarks, reports_count, created_by,
  } = await req.json();
  ...
  if (existing) {
    const { data, error } = await supabaseAdmin
      ...
      .update({
        ...
        reports_count:   isAbsent ? 0 : (reports_count || 0),
      })
      .eq('id', existing.id)
      .select()
      .single();
...
export async function PATCH(req: NextRequest) {
  const {
    id, doctor_id, department_id, entry_date,
    male_count, female_count, pediatric_count,
    status, remarks, reports_count,
  } = await req.json();
